...
**** visual line mode
    ...
    See Info node ‘Visual Line Mode’ for details.
...
*** emacs org-side-tree
  ...
  This can be helpful for navigation within an org file.

  I cannot tell whether this is better than "Shift Tab" to see all the headings within the org file.

  But it is good to know that it is out there.
...
*** which-key
  ...
  If you want to see the next page of which-key suggestions in the mini buffer, Ctrl X Ctrl h n. See https://github.com/justbur/emacs-which-key?tab=readme-ov-file#paging-options
...
*** Move around in a buffer quickly
     ...
     If so, move in a buffer more quickly with these commands.
     ...
     These commands are an alternative to using ~avy-goto-line~
...
*** Fix Paragraphs
  ...
  Make characters after a period capital case

  #+begin_src elisp
    (defun my-fix-paragraph-and-caps ()
    "Fill the paragraph and capitalize the start of every sentence."
    (interactive)
    (save-excursion
      ;; 1. Fill the paragraph (M-q)
      ;; (fill-paragraph)

      ;; 2. Move to the start of the paragraph
      (backward-paragraph)
      ;; 3. Find periods followed by a space and lowercase letter, then capitalize
      (while (re-search-forward "\\. +\\([a-z]\\)" (save-excursion (forward-paragraph) (point)) t)
  ...
  1. Evaluate the code above (or restart Emacs).
  2. Put your cursor in a messy paragraph.
  3. Run `M-x fix-paragraph-and-caps`.
